 ...
 */
export const trackAssetCache = (url: string) => {
  if (assetRegistry.has(url)) {
    console.log(`%c📦 [Asset Cache] HIT: ${url}`, 'color: #4CAF50; font-weight: bold;');
    return true;
  } else {
    console.log(`%c🌐 [Asset Cache] MISS (First Load): ${url}`, 'color: #2196F3; font-weight: bold;');
