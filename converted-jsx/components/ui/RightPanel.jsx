...
const MemoizedPropertyItem = memo(
  ({ item }) => {
    if (item.type === "basic") {
      return (
    ...
    return (
      <div className="px-4 py-1 flex justify-between gap-2 text-xs hover:bg-bim-bg-item-hover transition-colors">
        <span className="text-bim-text-muted shrink-0">{item.key}</span>
        <span
...
export const RightPanel = () => {
  const { rightPanelOpen, selectedElement, isHighlighting, toggleRightPanel } =
    useBIMStore(
      useShallow((s) => ({
          ...
          {selectedElement ? (
            <Virtuoso
              ...
              itemContent={(_index, item) => (
                <MemoizedPropertyItem item={item} />
              )}
            />
          ) : (
